...
//    @Digits(integer = 13, fraction = 0, message = "JMBG mora imati 13 cifara")
    ...
    @JoinTable(
            name = "studira",
            joinColumns = @JoinColumn(name = "studentID", referencedColumnName = "broj_indeksa"),
            inverseJoinColumns = @JoinColumn(name = "fakultetID", referencedColumnName = "maticni_broj"))
...
//        this.starost = LocalDate.now().getYear() - datumRodjenja.getYear();
    ...
    public List<Fakultet> getAssignedFakulteti() {
        return assignedFakulteti;
    }

    public void setAssignedFakulteti(List<Fakultet> assignedFakulteti) {
        this.assignedFakulteti = assignedFakulteti;
    }
...
//    public void ukloniFakultet(String fakultetId){
